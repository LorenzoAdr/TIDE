...
// Walk candidates (and parents) for a tuide source tree. Also probes /proc/self/exe.
std::optional<std::string> find_tuide_source_root(const std::vector<std::string>& search_roots);

// Merge-enable the bundle option for this server into ${tuide_root}/.bundle-config.
