...
bool language_allows_debounce_while_lsp_starting(const std::string& language_id) {
  return language_id_is_python(language_id) || language_id_is_shellscript(language_id) ||
         ...
         language_id_is_make(language_id) || language_id_is_yaml(language_id) ||
         language_id_is_xml(language_id);
}
...
bool any_lazy_lsp_client_ready(const std::unique_ptr<LspClient>& python_client,
                               ...
                               const std::unique_ptr<LspClient>& yaml_client,
                               const std::unique_ptr<LspClient>& xml_client) {
  return lazy_client_ready(python_client) || lazy_client_ready(bash_client) ||
         ...
         lazy_client_ready(make_client) || lazy_client_ready(yaml_client) ||
         lazy_client_ready(xml_client);
}
...
LspClient* LspSymbolProvider::client_for_path(const std::string& path) {
  ...
  if (language_id_is_yaml(lang)) {
    if (yaml_client_ && yaml_client_->ready()) {
      return yaml_client_.get();
    }
    return nullptr;
  }
  if (language_id_is_xml(lang)) {
    if (xml_client_ && xml_client_->ready()) {
      return xml_client_.get();
    }
    return nullptr;
  }
...
bool LspSymbolProvider::any_lsp_ready() const {
  ...
  return any_lazy_lsp_client_ready(python_client_, bash_client_, tex_client_, rust_client_,
                                   ...
                                   typescript_client_, cmake_client_, make_client_, yaml_client_, xml_client_);
}
...
void LspSymbolProvider::join_yaml_startup_thread() {
  join_lazy_startup_thread(yaml_lsp_startup_thread_, yaml_lsp_starting_);
}

void LspSymbolProvider::join_xml_startup_thread() {
  join_lazy_startup_thread(xml_lsp_startup_thread_, xml_lsp_starting_);
}
...
void LspSymbolProvider::finish_yaml_lsp_start_locked(bool ok, bool binary_missing) {
  finish_simple_lazy_lsp_start_locked(
      yaml_client_,
      {"lsp-yaml-start", "yaml", "status.yaml_ls_missing", "status.yaml_ls_failed",
       "status.yaml_ls_started", "yaml-language-server start failed or binary missing"},
      ok, binary_missing);
}

void LspSymbolProvider::finish_xml_lsp_start_locked(bool ok, bool binary_missing) {
  finish_simple_lazy_lsp_start_locked(
      xml_client_,
      {"lsp-xml-start", "xml", "status.lemminx_missing", "status.lemminx_failed",
       "status.lemminx_started", "lemminx start failed or binary missing"},
      ok, binary_missing);
}
...
void LspSymbolProvider::ensure_yaml_lsp_async() {
  ensure_simple_lazy_lsp_async(
      yaml_client_, yaml_lsp_starting_, yaml_lsp_startup_thread_,
      {"lsp-yaml-start", "yaml", "status.yaml_ls_missing", "status.yaml_ls_failed",
       "status.yaml_ls_started", "yaml-language-server start failed or binary missing",
       make_yaml_ls_spec},
      &LspSymbolProvider::finish_yaml_lsp_start_locked);
}

void LspSymbolProvider::ensure_xml_lsp_async() {
  ensure_simple_lazy_lsp_async(
      xml_client_, xml_lsp_starting_, xml_lsp_startup_thread_,
      {"lsp-xml-start", "xml", "status.lemminx_missing", "status.lemminx_failed",
       "status.lemminx_started", "lemminx start failed or binary missing",
       make_lemminx_spec},
      &LspSymbolProvider::finish_xml_lsp_start_locked);
}
...
void LspSymbolProvider::refresh_diagnostics_cache_locked() const {
  ...
  revision ^= client_diagnostics_revision(yaml_client_, 12);
  revision ^= client_diagnostics_revision(xml_client_, 14);
  revision ^= fortran_compiler_diag_revision_.load(std::memory_order_acquire) << 13;
  ...
  append_client_diagnostics(cached_diagnostics_, yaml_client_);
  append_client_diagnostics(cached_diagnostics_, xml_client_);
...
bool LspSymbolProvider::lsp_loading() const {
  return lsp_starting_.load(std::memory_order_acquire) ||
         ...
         yaml_lsp_starting_.load(std::memory_order_acquire) ||
         xml_lsp_starting_.load(std::memory_order_acquire);
}
...
bool LspSymbolProvider::yaml_lsp_ready() const {
  std::lock_guard<std::mutex> lock(mutex_);
  return yaml_client_ && yaml_client_->ready();
}

bool LspSymbolProvider::xml_lsp_ready() const {
  std::lock_guard<std::mutex> lock(mutex_);
  return xml_client_ && xml_client_->ready();
}
...
bool LspSymbolProvider::yaml_lsp_starting() const {
  return yaml_lsp_starting_.load(std::memory_order_acquire);
}

bool LspSymbolProvider::xml_lsp_starting() const {
  return xml_lsp_starting_.load(std::memory_order_acquire);
}
...
void LspSymbolProvider::stop_lsp() {
  ...
  LspClient* yaml = nullptr;
  LspClient* xml = nullptr;
  {
    std::lock_guard<std::mutex> lock(mutex_);
  ...
  join_thread_if_joinable(yaml_lsp_startup_thread_);
  yaml_lsp_starting_.store(false, std::memory_order_release);
  join_thread_if_joinable(xml_lsp_startup_thread_);
  xml_lsp_starting_.store(false, std::memory_order_release);
  {
    std::lock_guard<std::mutex> life(lifecycle_mutex_);
...
void LspSymbolProvider::set_diagnostics_notify_callback(
  ...
  if (yaml_client_) {
    yaml_client_->set_diagnostics_notify_callback(callback);
  }
  if (xml_client_) {
    xml_client_->set_diagnostics_notify_callback(callback);
  }
}
...
void LspSymbolProvider::set_lsp_request_counter(std::atomic<uint64_t>* counter) {
  ...
  if (yaml_client_) {
    yaml_client_->set_request_counter(counter);
  }
  if (xml_client_) {
    xml_client_->set_request_counter(counter);
  }
}
...
void LspSymbolProvider::set_ui_inhibited(const bool inhibited) {
  ...
  if (should_pause) {
    ...
    if (yaml_client_) {
      yaml_client_->set_background_paused(true);
    }
    if (xml_client_) {
      xml_client_->set_background_paused(true);
    }
    return;
  }
  ...
  if (yaml_client_) {
    yaml_client_->set_background_paused(false);
  }
  if (xml_client_) {
    xml_client_->set_background_paused(false);
  }
  if (should_flush) {
...
void LspSymbolProvider::on_document_opened(const std::string& path, const std::string& text) {
  ...
  if (is_lazy_lsp_language(lang)) {
    ...
    } else if (language_id_is_yaml(lang)) {
      ensure_yaml_lsp_async();
    } else if (language_id_is_xml(lang)) {
      ensure_xml_lsp_async();
    }
  }
  if (ensure_open) {
    if (LspClient* lsp = client_for_path(path)) {
...
std::vector<SymbolInfo> LspSymbolProvider::workspace_symbols(const std::string& workspace_root,
  ...
  append_workspace_symbols(out, yaml_client_, workspace_root_, query);
  append_workspace_symbols(out, xml_client_, workspace_root_, query);
  return out;
}
...
bool LspSymbolProvider::request_completion(const CompletionParams& params,
  ...
  cancel_client_completion(yaml_client_);
  cancel_client_completion(xml_client_);
...
std::optional<std::vector<CompletionItem>> LspSymbolProvider::poll_completion(
  ...
  const int yaml_latest = client_latest_completion_id(yaml_client_);
  const int xml_latest = client_latest_completion_id(xml_client_);
  if (it->second.request_id <= 0 ||
      (it->second.request_id != clangd_latest && it->second.request_id != py_latest &&
       ...
       it->second.request_id != yaml_latest && it->second.request_id != xml_latest)) {
