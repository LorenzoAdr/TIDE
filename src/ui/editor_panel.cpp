...
struct EditorPanelState {
  ...
  uint64_t last_ts_baseline_commit_token = 0;
  std::unordered_set<int> ts_dirty_highlight_lines;
  ...
  Box diff_action_box;
  std::vector<int> diff_revert_block_by_viewport_row;
  int diff_revert_hover_block = -1;
  int diff_revert_pressed_block = -1;
};
...
uint64_t compute_viewport_line_render_key(const ViewportLineRenderKeyInput& line,
                                        ...
                                        bool editor_focused, bool show_caret,
  ...
  if (buffer.semantic_layout_dirty) {
    if (!kLspSemanticTokensStandby) {
      ...
      panel->semantic_tokens_layout_stale_from_line =
          std::min(panel->semantic_tokens_layout_stale_from_line,
...
void editor_hover_tick(WorkspaceModel* workspace, EditorPanelState* panel,
  ...
  if (layout_state != nullptr && layout_state->pending_symbol_info.active) {
    const auto req = layout_state->pending_symbol_info;
    layout_state->pending_symbol_info.active = false;
    trigger_lsp_hover_at(panel, CursorPos{req.line, req.col}, req.anchor_x, req.anchor_y);
  }
...
bool scroll_tabular_lines(WorkspaceModel* workspace, EditorPanelState* panel, int delta_lines,
                          int visible_lines);
bool virtual_text_view_ready(WorkspaceModel* workspace, EditorPanelState* panel);
bool scroll_virtual_lines(WorkspaceModel* workspace, EditorPanelState* panel, int delta_lines,
                          int visible_lines);
...
int virtual_max_allowed_scroll(EditorPanelState* panel, int visible_lines);
void clamp_virtual_scroll(EditorPanelState* panel, EditorBuffer* buffer, int visible_lines);
void maybe_request_virtual_chunk(EditorPanelState* panel, EditorBuffer* buffer, int visible_lines);
...
bool handle_overview_ruler_mouse(WorkspaceModel* workspace, FocusManagerState* focus,
  ...
  if (m.button == Mouse::WheelUp) {
    if (scroll_virtual_lines(workspace, panel, -3, visible_lines)) {
      clear_hover_state(&panel->hover);
      return true;
    }
...
bool apply_scrollbar_drag(WorkspaceModel* workspace, EditorPanelState* panel, int mouse_y,
  ...
  } else if (virtual_text_view_ready(workspace, panel)) {
    new_scroll = std::min(new_scroll, virtual_max_allowed_scroll(panel, visible_lines));
  }
  ...
  if (m.motion == Mouse::Pressed) {
    ...
    if (scrollbar_thumb_hit(panel->scrollbar_layout, panel->scrollbar_box, m.x, m.y)) {
      ...
      if (tabular_view_ready(panel, workspace->buffer)) {
        ...
        maybe_request_virtual_chunk(panel, &workspace->buffer, visible_lines);
      }
      panel->scrollbar_dragging = true;
...
bool handle_fold_gutter_click(EditorPanelState* panel, EditorBuffer* buffer, const Mouse& m,
  ...
  stabilize_scroll_after_fold_change(buffer, buffer->fold_regions, visible_lines);
  return true;
}
...
bool is_member_access_at_cursor(const EditorBuffer& buffer) {
  ...
  if (col >= 2 && text[static_cast<std::size_t>(col - 2)] == '-' && prev == '>') {
    return true;
  }
...
bool completion_ranked_before(const CompletionRankedCandidate& a,
  ...
  if (cache[a.index].source_index != cache[b.index].source_index) {
...
void sync_completion_word_typed_after_lsp(CompletionState* completion);
...
struct CompletionState {
  ...
  int lsp_fetch_col = 0;
  int word_typed_chars = 0;
  std::string lsp_last_fetched_scope_key;
  ...
  void clear_suggestion_cache(EditorPanelState* panel) {
    lsp_items.clear();
    ...
    semantic_mode = false;
    if (panel != nullptr) {
      panel->scope_completion_cache_key.clear();
  ...
  void sync_scope_cache(const EditorBuffer& buffer, EditorPanelState* panel) {
    if (kDisableLspCompletionScopeCacheForTest) {
      return;
    }
  ...
  bool apply_polled_lsp_response(const std::string& key, std::vector<CompletionItem> items,
    ...
    if (key.empty() || key != lsp_pending_key) {
      return false;
    }
    const std::string resolved_key = lsp_pending_key;
    lsp_inflight_key.clear();
    lsp_items = std::move(items);
    lsp_resolved_query = query;
    lsp_resolved_key = resolved_key;
    if (!lsp_items.empty()) {
    ...
    invalidate_item_cache();
    sync_completion_word_typed_after_lsp(this);
    return true;
  }
  ...
  void rebuild_item_cache() {
    ...
    } else if (!kDisableLspCompletionScopeCacheForTest) {
...
bool lsp_trigger_from_typed_char(char typed, const EditorBuffer& buffer) {
  if (typed == ' ') {
    return false;
  }
  if (typed == '.' || typed == '>' || typed == ':') {
    return is_member_access_at_cursor(buffer);
  }
  return false;
}

bool should_trigger_lsp_fetch(const EditorBuffer& buffer, const CompletionState& completion,
                              char typed = '\0') {
  if (typed == '\0') {
    return false;
  }
  ...
  if (lsp_trigger_from_typed_char(typed, buffer)) {
    return true;
  }
  if (is_ident_char(typed)) {
    return completion.word_typed_chars == 2;
  }
  return false;
}

bool live_lsp_fetch_pending(const CompletionState& completion) {
  return completion.live_mode && !completion.lsp_pending_key.empty() &&
         completion.lsp_pending_key != completion.lsp_resolved_key;
}

std::string completion_anchor_key_for_buffer(const EditorBuffer& buffer) {
  int replace_start = 0;
  int replace_end = 0;
  if (!completion_replace_range_at_cursor(buffer, buffer.primary(), &replace_start,
                                          &replace_end)) {
    return {};
  }
  return completion_anchor_key(buffer.path, buffer.primary().head.line, replace_start);
}

bool completion_serving_cached_lsp_for_anchor(const CompletionState& completion,
                                                const std::string& anchor_key) {
  if (anchor_key.empty() || completion.lsp_items.empty()) {
    return false;
  }
  return completion.lsp_resolved_key == anchor_key ||
...
void sync_completion_word_typed_after_lsp(CompletionState* completion) {
  ...
  completion->word_typed_chars = std::max(
      completion->word_typed_chars, static_cast<int>(completion->prefix.size()));
}

void clear_lsp_completion_for_scope_change(CompletionState* completion,
                                           const std::shared_ptr<ISymbolProvider>& symbols,
                                           EditorPanelState* panel,
                                           MainLayoutState* layout_state) {
  if (symbols != nullptr) {
    symbols->cancel_completion_fetch();
  }
  if (completion == nullptr) {
    return;
  }
  completion->clear_suggestion_cache(panel);
  ...
  completion->lsp_last_fetched_scope_key.clear();
...
void cancel_live_lsp_on_cursor_move(CompletionState* completion,
                                    const std::shared_ptr<ISymbolProvider>& symbols,
                                    MainLayoutState* layout_state,
                                    EditorPanelState* panel = nullptr) {
  if (completion == nullptr && symbols == nullptr && panel == nullptr) {
    return;
  }
  if (symbols != nullptr) {
    symbols->cancel_completion_fetch();
    symbols->cancel_hover_fetch();
  }
  if (panel != nullptr) {
    clear_hover_state(&panel->hover);
  }
...
void reveal_completion_after_lsp_poll(CompletionState* completion,
                                      MainLayoutState* layout_state,
                                      const std::shared_ptr<ISymbolProvider>& symbols,
  ...
  sync_completion_word_typed_after_lsp(completion);
...
constexpr int kVirtualTsContextLines = 48;

}  // namespace

bool virtual_text_view_ready(WorkspaceModel* workspace, EditorPanelState* panel) {
  return workspace != nullptr && workspace->active_tab_large_virtual_view() && panel != nullptr &&
         panel->virtual_store != nullptr && panel->virtual_store->ready();
}

int virtual_max_allowed_scroll(EditorPanelState* panel, int visible_lines) {
  if (panel == nullptr || panel->virtual_store == nullptr || !panel->virtual_store->ready()) {
    return 0;
  }
...
void clamp_virtual_scroll(EditorPanelState* panel, EditorBuffer* buffer, int visible_lines) {
  if (panel == nullptr || buffer == nullptr || panel->virtual_store == nullptr ||
      !panel->virtual_store->ready()) {
    return;
  }
  const int allowed = virtual_max_allowed_scroll(panel, visible_lines);
  buffer->scroll = std::max(0, std::min(buffer->scroll, allowed));
}
...
void maybe_request_virtual_chunk(EditorPanelState* panel, EditorBuffer* buffer, int visible_lines) {
  ...
  if (!store->has_more() || store->loading_more() || panel->virtual_scroll_locked) {
    return;
  }
  ...
  if (store->request_load_at_end(buffer->scroll, visible_lines)) {
    panel->virtual_scroll_locked = true;
    panel->virtual_scroll_limit = buffer->scroll;
  }
}
...
void ensure_virtual_viewport_ts(const std::string& path, VirtualTextFileStore* store, int scroll,
  ...
  const int view_end = std::min(total - 1, scroll + visible_lines - 1);
  const int first = std::max(0, scroll - kVirtualTsContextLines);
  const int last = std::min(total - 1, view_end + kVirtualTsContextLines);
  std::string slice;
  slice.reserve(static_cast<std::size_t>((last - first + 1) * 64));
  for (int line = first; line <= last; ++line) {
    if (!slice.empty()) {
      slice.push_back('\n');
    }
    slice += store->line_at(line);
  }
  if (slice.empty()) {
    return;
  }
  tree_sitter_service().ensure_viewport_preview_slice(path, first, last, slice);
}
...
bool completion_awaiting_async_lsp(const CompletionState* completion,
  ...
  if (!completion->open && !live_lsp_fetch_pending(*completion)) {
    return false;
  }
  ...
  if (!completion->lsp_inflight_key.empty() && completion->lsp_last_request_ms > 0 &&
      now_ms > completion->lsp_last_request_ms + wait_ms) {
    return false;
  }
...
bool completion_ui_show_loading(const CompletionState* completion,
  ...
  if (completion->lsp_last_request_ms <= 0) {
    return true;
  }
  const int64_t now_ms = steady_now_ms();
  const int64_t wait_ms =
      live_completion_lsp_wait_timeout_ms(buffer_text, completion->lsp_fetch_line);
  return now_ms <= completion->lsp_last_request_ms + wait_ms;
}
...
void schedule_completion_lsp_fetch(CompletionState* completion, WorkspaceModel* workspace,
  ...
  if (!kDisableLspCompletionScopeCacheForTest && live_only && !completion->lsp_items.empty() &&
  ...
  if (completion->lsp_resolved_key == anchor_key && completion->lsp_items.empty()) {
    completion->lsp_resolved_key.clear();
    completion->lsp_resolved_query.clear();
  }
  completion->lsp_pending_key = anchor_key;
...
bool try_poll_lsp_completion(CompletionState* completion, WorkspaceModel* workspace,
                             const std::shared_ptr<ISymbolProvider>& symbols,
                             EditorPanelState* panel, std::string key) {
  if (completion == nullptr || workspace == nullptr || symbols == nullptr || key.empty()) {
    return false;
  }
  if (const auto polled = symbols->poll_completion(key)) {
    workspace->ensure_buffer();
    const bool applied = completion->apply_polled_lsp_response(key, std::move(*polled),
                                                               workspace->buffer, panel);
    if (applied) {
      prepare_completion_at_cursor(completion, &workspace->buffer, panel);
    }
    return applied;
  }
  return false;
...
void completion_lsp_tick(CompletionState* completion, WorkspaceModel* workspace,
  ...
  const std::string live_anchor_key = completion_anchor_key_for_buffer(buffer);
  if (!live_anchor_key.empty() &&
      completion_serving_cached_lsp_for_anchor(*completion, live_anchor_key) &&
      completion->lsp_inflight_key.empty()) {
    return;
  }
  ...
  if (completion_uses_async_lsp(layout_state, symbols, buffer_path) &&
      !completion->lsp_inflight_key.empty() && completion->lsp_last_request_ms > 0 &&
      now_ms > completion->lsp_last_request_ms + lsp_wait_ms) {
    if (!try_poll_lsp_completion(completion, workspace, symbols, panel,
                                 completion->lsp_inflight_key)) {
      completion->lsp_inflight_key.clear();
      completion->lsp_pending_key.clear();
      completion->lsp_last_request_ms = 0;
      completion->refresh_matches();
      ...
      reveal_completion_after_lsp_poll(completion, layout_state, symbols, buffer_path,
                                     buffer_text_snapshot);
    }
    return;
  }
  ...
  if (completion->lsp_inflight_key.empty()) {
    if (!kDisableLspCompletionScopeCacheForTest && !completion->lsp_pending_key.empty() &&
        completion->lsp_pending_key == completion->lsp_resolved_key &&
  ...
  if (live) {
    workspace->ensure_buffer();
    const EditorBuffer& live_buffer = workspace->buffer;
    if (completion->word_typed_chars < 2 && completion->lsp_pending_key.empty() &&
        !is_member_access_at_cursor(live_buffer)) {
      return;
    }
    if (live_buffer.primary_line() != completion->lsp_fetch_line) {
...
void maybe_schedule_lsp_on_type(CompletionState* completion, WorkspaceModel* workspace,
                                const std::shared_ptr<ISymbolProvider>& symbols,
  ...
  if (typed == ' ' || typed == '(') {
    completion->word_typed_chars = 0;
    cancel_live_lsp_on_cursor_move(completion, symbols, layout_state, panel);
    return;
  }

  const bool member_trigger = lsp_trigger_from_typed_char(typed, *buffer);
  if (member_trigger) {
    completion->word_typed_chars = 0;
    clear_lsp_completion_for_scope_change(completion, symbols, panel, layout_state);
  } else if (is_ident_char(typed)) {
    completion->word_typed_chars++;
  }
  if (!member_trigger && !is_ident_char(typed)) {
    return;
  }
  ...
  const std::string anchor_key =
      completion_anchor_key(buffer->path, completion->replace_line, completion->replace_start);
  const bool cached = completion_serving_cached_lsp_for_anchor(*completion, anchor_key);
  const bool should_trigger = should_trigger_lsp_fetch(*buffer, *completion, typed);

  if ((completion->open || cached) && !member_trigger) {
    completion->refresh_matches();
    if (completion->open) {
  ...
  if (live_lsp_fetch_pending(*completion)) {
    return;
  }
...
void update_open_completion_on_edit(CompletionState* completion, WorkspaceModel* workspace,
                                    const std::shared_ptr<ISymbolProvider>& symbols,
                                    MainLayoutState* layout_state, EditorBuffer* buffer,
                                    EditorPanelState* panel = nullptr) {
  if (completion == nullptr || workspace == nullptr || buffer == nullptr) {
    return;
  }
  if (!completion->open) {
    if (live_lsp_fetch_pending(*completion)) {
      buffer->ensure_cursors();
      if (word_at_cursor(*buffer, buffer->primary()).size() < 2) {
        cancel_live_lsp_on_cursor_move(completion, symbols, layout_state, panel);
      }
    }
    return;
...
void maybe_open_live_completion(CompletionState* completion, WorkspaceModel* workspace,
                                ...
                                EditorPanelState* panel, char typed) {
  (void)symbol_indexer;
  maybe_schedule_lsp_on_type(completion, workspace, symbols, layout_state, buffer, panel, typed);
}
...
bool handle_editor_escape(EditorBuffer* buffer, EditorFindState* find,
  ...
  if (panel != nullptr) {
    ...
    clear_snippet_session(&panel->snippet_session);
  }
...
void autoscroll_on_drag(EditorBuffer* buffer, const Mouse& m, const EditorPanelState& panel,
  ...
  if (!panel.code_box.IsEmpty()) {
    const int box_height = panel.code_box.y_max - panel.code_box.y_min + 1;
    const int local_y = m.y - panel.code_box.y_min;
    const int margin = kScrollEdgeMarginLines;
    const int total = static_cast<int>(buffer->lines.size());
    const int max_scroll_line = max_scroll(total, visible_lines);

    if (m.y < panel.code_box.y_min) {
      scroll_view_by_lines(buffer, -1, visible_lines);
    } else if (local_y < margin && buffer->scroll > 0) {
      scroll_view_by_lines(buffer, -1, visible_lines);
    } else if (m.y > panel.code_box.y_max) {
      scroll_view_by_lines(buffer, 1, visible_lines);
    } else if (local_y >= box_height - margin && buffer->scroll < max_scroll_line) {
      scroll_view_by_lines(buffer, 1, visible_lines);
    }
  }
...
constexpr int kGitDiffActionWidth = 2;

int diff_revert_block_at_viewport_row(const EditorPanelState& panel, int viewport_row) {
  if (viewport_row < 0 ||
      viewport_row >= static_cast<int>(panel.diff_revert_block_by_viewport_row.size())) {
    return -1;
  }
  return panel.diff_revert_block_by_viewport_row[static_cast<std::size_t>(viewport_row)];
}

int diff_revert_block_for_row_index(const std::vector<SideBySideDiffRow>& rows, int row_index) {
  ...
  for (int block_index = 0; block_index < static_cast<int>(blocks.size()); ++block_index) {
    ...
    if (row_index >= block.start_row && row_index <= block.end_row) {
      return block_index;
    }
  }
  return -1;
...
bool revert_git_diff_block_from_panel(WorkspaceModel* workspace, EditorPanelState* panel,
                                      GitService* git, int block_index,
                                      MainLayoutState* layout_state) {
  if (workspace == nullptr || panel == nullptr || git == nullptr || block_index < 0) {
    return false;
  }
  if (!workspace->revert_git_diff_block(block_index, git)) {
    return false;
  }
  panel->git_cache_revision = 0;
  panel->git_cache_view_token = 0;
  panel->diff_revert_hover_block = -1;
  panel->diff_revert_pressed_block = -1;
  if (layout_state != nullptr) {
    UI_WAKE(layout_state, "git.diff.revert");
  }
  return true;
}

bool handle_git_diff_view_mouse(WorkspaceModel* workspace, FocusManagerState* focus,
                                EditorPanelState* panel, GitService* git,
                                MainLayoutState* layout_state, const Mouse& m, int visible_lines) {
  if (workspace == nullptr || panel == nullptr || !workspace->active_tab_git_diff_view()) {
    return false;
  }
  ...
  if (m.motion == Mouse::Moved) {
    if (!in_code) {
      if (panel->diff_revert_hover_block >= 0) {
        panel->diff_revert_hover_block = -1;
        if (layout_state != nullptr) {
          UI_WAKE(layout_state, "git.diff.hover");
        }
      }
      return false;
    }
    const int local_x = m.x - panel->code_box.x_min;
    const int local_y = m.y - panel->code_box.y_min;
    int hover_block = -1;
    if (local_x >= 0 && local_x < kGitDiffActionWidth && local_y > 0) {
      hover_block = diff_revert_block_at_viewport_row(*panel, local_y - 1);
    }
    if (hover_block != panel->diff_revert_hover_block) {
      panel->diff_revert_hover_block = hover_block;
      if (layout_state != nullptr) {
        UI_WAKE(layout_state, "git.diff.hover");
      }
    }
    return hover_block >= 0;
  }

  if (m.button == Mouse::Left && in_code) {
    const int local_x = m.x - panel->code_box.x_min;
    const int local_y = m.y - panel->code_box.y_min;
    if (local_x >= 0 && local_x < kGitDiffActionWidth && local_y > 0) {
      const int block = diff_revert_block_at_viewport_row(*panel, local_y - 1);
      if (block < 0) {
        return true;
      }
      claim_editor_focus(focus, layout_state, panel->panel_focus);
      if (m.motion == Mouse::Pressed) {
        panel->diff_revert_pressed_block = block;
        if (layout_state != nullptr) {
          UI_WAKE(layout_state, "git.diff.press");
        }
        return true;
      }
      if (m.motion == Mouse::Released &&
          panel->diff_revert_pressed_block == block) {
        revert_git_diff_block_from_panel(workspace, panel, git, block, layout_state);
        panel->diff_revert_pressed_block = -1;
        return true;
      }
      return true;
...
bool handle_editor_mouse(WorkspaceModel* workspace, FocusManagerState* focus,
  ...
  const auto& m = event.mouse();
  if (workspace->active_tab_git_diff_view() &&
      handle_git_diff_view_mouse(workspace, focus, panel, git, layout_state, m, visible_lines)) {
    return true;
  }
...
bool accept_completion(CompletionState* completion, EditorBuffer* buffer,
  ...
  const bool is_callable = item.kind == SymbolKind::kFunction || item.kind == SymbolKind::kMethod;
  ...
  if (treat_as_snippet) {
    ...
    snippet = finalize_function_call_insert(raw_insert, item.detail, is_callable, paren_already_there);
  }

  if (buffer->multi_cursor_active()) {
    apply_completion_at_all_cursors(buffer, snippet);
    clear_snippet_session(panel != nullptr ? &panel->snippet_session : nullptr);
  } else {
  ...
  if (event.is_character()) {
    ...
    if (ch.size() == 1 && static_cast<unsigned char>(ch[0]) >= 32 &&
      ...
      if (is_ident_char(typed)) {
        ...
        update_open_completion_on_edit(completion, workspace, symbols, layout_state, buffer,
                                       panel);
        return true;
      }
      completion->query += ch;
...
HelixDispatchContext build_helix_dispatch_context(WorkspaceModel* workspace, EditorPanelState* panel,
  ...
  ctx.snippet_session = panel != nullptr ? &panel->snippet_session : nullptr;
  return ctx;
}

bool helix_editor_active(MainLayoutState* layout_state, bool tabular_view, bool virtual_view) {
...
bool handle_editor_keys(WorkspaceModel* workspace, FocusManagerState* focus,
  ...
  if (event_is_ctrl_backspace(event)) {
    ...
    notify_editor_buffer_changed(workspace, panel, symbols, layout_state);
    if (completion != nullptr && completion->open) {
      update_open_completion_on_edit(completion, workspace, symbols, layout_state, buffer, panel);
    }
    return true;
  }
...
Component MakeEditorPanel(WorkspaceModel* workspace, FocusManagerState* focus,
  ...
  auto code_view = Renderer([workspace, focus, panel_state, find_state, symbols, layout_state,
    ...
    if (path_changed) {
      ...
      panel_state->virtual_store.reset();
      panel_state->virtual_scroll_locked = false;
      panel_state->virtual_scroll_limit = 0;
      panel_state->virtual_ts_source_key.clear();
    ...
    VirtualTextFileStore* virtual_store =
        large_virtual_view ? panel_state->virtual_store.get() : nullptr;
    ...
    const bool virtual_ready = virtual_store != nullptr && virtual_store->ready();
    const bool virtual_indexing = virtual_store != nullptr && virtual_store->indexing();

    int total = static_cast<int>(buffer.lines.size());
    if (virtual_ready) {
      total = virtual_store->line_count();
    }
    ...
    if (large_virtual_view && virtual_store != nullptr) {
      if (layout_state != nullptr) {
        const int64_t now = steady_now_ms();
        if (virtual_indexing || virtual_store->loading_more() ||
            (virtual_ready && virtual_store->has_more())) {
          if (now - panel_state->last_virtual_index_tick_ms >= 200) {
            panel_state->last_virtual_index_tick_ms = now;
            UI_WAKE(layout_state, "schedule");
          }
        }
      }
      if (virtual_store->loading_more() && virtual_ready) {
        panel_state->virtual_scroll_locked = false;
      }
    }
    ...
    if (diff_view) {
      ...
      const GitDiffViewRenderResult diff_render = render_git_diff_viewport(
          ...
          buffer.primary_line(), panel_state->diff_revert_hover_block,
          panel_state->diff_revert_pressed_block);
      panel_state->diff_revert_block_by_viewport_row = diff_render.revert_block_by_viewport_row;
    ...
    if (large_virtual_view && virtual_store != nullptr) {
      ...
      if (virtual_indexing) {
        gutter_rows.push_back(text(format_line_number(1, 4)) | color(theme::Muted()) | row_bg);
        code_rows.push_back(text(i18n::tr("editor.virtual.indexing")) | color(theme::Muted()) |
                            row_bg);
      } else if (!virtual_ready) {
        gutter_rows.push_back(text(format_line_number(1, 4)) | color(theme::Muted()) | row_bg);
        const std::string message =
            virtual_store->error_message().empty()
                ? i18n::tr("editor.virtual.open_failed")
                : virtual_store->error_message();
        code_rows.push_back(text(message) | color(theme::Error()) | row_bg);
      } else {
        ...
        if (indexed_virtual) {
          ensure_virtual_viewport_ts(buffer.path, virtual_store, scroll, visible);
        }

        SyntaxHighlightContext virtual_hl_ctx;
        virtual_hl_ctx.file_path = buffer.path;
        virtual_hl_ctx.viewport_only_ts = indexed_virtual;
        virtual_hl_ctx.joined_override = &panel_state->virtual_ts_source_key;
        virtual_hl_ctx.buffer_token = buffer.view_token;
        virtual_hl_ctx.ts_revision = ts_highlight_revision;
        virtual_hl_ctx.line_span_cache = &panel_state->line_syntax_span_cache;

        int max_line_len = 0;
        const int view_end = std::min(file_lines, scroll + visible);
        ...
        if (virtual_store->loading_more()) {
          gutter_rows.push_back(text(format_line_number(view_end + 1, gutter_w)) |
                              color(theme::Muted()) | row_bg);
          code_rows.push_back(text(i18n::tr("editor.virtual.loading_lines")) | color(theme::Muted()) |
                              row_bg);
        }

        const int max_scroll_col = std::max(0, max_line_len - code_width + 1);
        ...
        const bool scroll_active =
            ...
            (layout_state != nullptr &&
             layout_state->clickable.is_pressed(press_id::kEditorScrollbar));
        Element scrollbar =
            vertical_scrollbar(file_lines, scroll, visible, rendered_lines, scroll_hovered,
                               scroll_active) |
            reflect(panel_state->scrollbar_box);
        panel_state->scrollbar_layout =
            compute_scrollbar_layout(file_lines, scroll, visible, rendered_lines);
        const bool h_scroll_hovered =
            layout_state != nullptr &&
            layout_state->clickable.is_hovered(press_id::kEditorHorizontalScrollbar);
        const bool h_scroll_active =
        ...
        const int total_content_width =
            max_scroll_col > 0 ? max_scroll_col + code_width : code_width;
        Element code_column = attach_horizontal_scrollbar(
            std::move(code), total_content_width, buffer.scroll_col, code_width, h_scroll_hovered,
            h_scroll_active, panel_state.get());
        return hbox({gutter, separator() | color(theme::AccentDim()), std::move(code_column),
    ...
    highlight_ctx.syntax_incremental = caret_live_highlight;
    highlight_ctx.editing_line = caret_live_highlight ? caret_line_index : -1;
    highlight_ctx.dirty_highlight_lines = &panel_state->ts_dirty_highlight_lines;
    if (indexed_cpp && editor_content_settled(*panel_state) && ts_highlights_pending &&
        panel_state->last_ts_baseline_commit_token != buffer.view_token) {
      std::unordered_set<int> lines_to_commit = panel_state->ts_dirty_highlight_lines;
      if (caret_line_index >= 0) {
        lines_to_commit.insert(caret_line_index);
      }
      for (int dirty_line : lines_to_commit) {
        if (dirty_line >= 0 && dirty_line < static_cast<int>(buffer.lines.size())) {
          tree_sitter_service().commit_line_highlights(buffer.path, buffer_source, dirty_line,
                                                       buffer.lines.at(dirty_line));
        }
      }
      panel_state->last_ts_baseline_commit_token = buffer.view_token;
      panel_state->line_syntax_span_cache.clear();
    ...
    }
    {
      ...
      const SemanticTokenDocument* line_semantic =
          caret_or_dirty_ts_live || line_shifted_by_layout_change ? nullptr : semantic_tokens;
      ...
      const uint64_t render_key = compute_viewport_line_render_key(
          key_input, buffer, *panel_state, editor_focused, show_caret, helix_caret_insert,
          buffer.scroll_col, code_width,
  ...
  auto dispatch_editor_keys = [workspace, focus, panel_state, tab_bar_state, find_state,
                               ...
                               git_service, symbols, file_indexer, symbol_indexer, layout_state,
                               debug_model, on_command, panel_focus](Event event) {
    if (tab_bar_state->overflow_open &&
    ...
    if (workspace->active_tab_git_diff_view() && event.character() == "r" &&
        git_service != nullptr) {
      int block = panel_state->diff_revert_hover_block;
      if (block < 0) {
        block = diff_revert_block_for_row_index(workspace->active_diff_rows(),
                                                workspace->buffer.primary_line());
      }
      if (block >= 0) {
        revert_git_diff_block_from_panel(workspace, panel_state.get(), git_service, block,
                                         layout_state);
      }
      return true;
    }
