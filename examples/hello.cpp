...
bool send_packet(int fd, const void* data, std::size_t size) {
  ...
  const ssize_t sent = sendto(fd, data, size, 0, reinterpret_cast<sockaddr*>(&dest), sizeof(dest));
...
void try_receive(int fd) {
  ...
  const ssize_t received =
      recvfrom(fd, buffer.data(), buffer.size(), MSG_DONTWAIT, reinterpret_cast<sockaddr*>(&src),
               &src_len);
  if (received > 0) {
