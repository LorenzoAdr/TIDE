...
void check_indices(const tuide::FuzzyMatchResult& result, std::initializer_list<std::size_t> expected) {
...
void test_empty_query_matches() {
  const auto result = tuide::fuzzy_match("src/main.cpp", "");
...
void test_subsequence_match() {
  const auto result = tuide::fuzzy_match("MyApp.cpp", "mapp");
  check(result.matched, "mapp matches MyApp.cpp");
  check(!result.indices.empty(), "has indices");
}

void test_path_subsequence() {
  const auto result = tuide::fuzzy_match("src/editor/application.cpp", "edap");
  check(result.matched, "edap matches path");
}

void test_no_match() {
  const auto result = tuide::fuzzy_match("main.cpp", "xyz");
  check(!result.matched, "xyz does not match");
}

void test_case_insensitive() {
  const auto result = tuide::fuzzy_match("Strings_EN.cpp", "str_en");
  check(result.matched, "case insensitive subsequence");
}

void test_consecutive_scores_higher() {
  const auto early = tuide::fuzzy_match("main.cpp", "main");
  const auto late = tuide::fuzzy_match("not_main.cpp", "main");
...
void test_separator_bonus() {
  const auto result = tuide::fuzzy_match("src/app/main.cpp", "sam");
...
void test_cached_matches_plain_api() {
  ...
  const auto plain = tuide::fuzzy_match(haystack, query);
  const auto cached =
      tuide::fuzzy_match_cached(haystack, tuide::fuzzy_to_lower(haystack), tuide::fuzzy_to_lower(query));
...
void test_fuzzy_to_lower() {
  check(tuide::fuzzy_to_lower("Src/App/Main.CPP") == "src/app/main.cpp", "lower cache");
}
