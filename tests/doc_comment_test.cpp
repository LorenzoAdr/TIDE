...
void test_extract_cpp_params() {
  const auto names =
      tuide::extract_param_names("int foo(const std::string& bar, int x = 0)", "cpp");
...
void test_extract_python_params() {
  const auto names =
      tuide::extract_param_names("def foo(self, bar: int, x=1):", "python");
...
void test_cpp_doc_comment_snippet() {
  tuide::DocCommentRequest req;
  req.path = "foo.cpp";
  req.kind = tuide::SymbolKind::kFunction;
  req.symbol_name = "foo";
  req.declaration_line = "void foo(int a, double b)";
  req.indent_cols = 0;
  const std::string snippet = tuide::build_doc_comment_snippet(req);
...
void test_python_doc_comment_snippet() {
  ...
  const tuide::DocCommentInsertPlan plan = tuide::plan_doc_comment_insert(req, 10);
  expect(plan.insert_line == 11, "python inserts below def");
}

void test_separator_snippet() {
  const std::string snippet = tuide::build_separator_snippet("foo.cpp", 2);
...
void test_file_header_banner() {
  const std::string snippet = tuide::build_file_header_snippet("src/widget.cpp");
  ...
  const std::string py = tuide::build_file_header_snippet("mod.py");
