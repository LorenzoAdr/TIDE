...
peek: visual highlight apply visual highlight fold regions, visual highlight compute, visual highlight drain visual highlight results
port: M1=>M2 editor panel MakeEditorPanel -write-> scroll
M2  kind=chrome  ov=15  stems: editor panel editor panel
owns: editor panel
nucleus: scroll
peek: editor panel make breadcrumb bar, editor panel MakeEditorPanel, editor panel handle breadcrumb click
...
peek: application sync symbol workspace indexer, application set workspace, application request ai indexes
...
peek: console panel MakeConsolePanel, console panel open terminal link, console panel toggle console expanded
...
peek: watches panel MakeWatchesPanel, watches panel rebuild flat watches, watches panel handle breakpoint hw input
...
owns: llama backend
gap: no state
peek: model store llama bundle dir for, model store runtime dir, model store ensure llama bundle
peek-edge: model store llama bundle dir for -call-> model store runtime dir
M9  same=M5  kind=hole  ov=9  stems: application
peek: workspace model open new tab from disk, workspace model save buffer, workspace model stamp tab disk mtime
...
peek: diagnostics panel MakeDiagnosticsPanel, symbol provider diagnostics revision, symbol provider supports diagnostics
...
gap: no state
peek: language server spec make texlab spec, language server spec make lemminx spec, language server spec make lua ls spec
peek-edge: language server spec make texlab spec -write-> command
M12  same=M5  kind=hole  ov=12  stems: application
peek: file picker MakeFilePickerOverlay, symbol picker MakeSymbolPickerOverlay, external file wizard MakeExternalFileWizardOverlay
