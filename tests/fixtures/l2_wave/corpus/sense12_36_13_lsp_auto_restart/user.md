...
1. [en_curso] A locator: dónde se llama a restart lsp for workspace cuando el proceso LSP muere
2. [pendiente] B locator: dónde se llama a schedule debounced lsp restart cuando el proceso LSP muere
3. [pendiente] P puente: hay camino la detección de muerte del proceso y la llamada al reinicio (need A B)
...
T3 preguntó:
dónde se detecta que el proceso del servidor LSP ha salido o muerto para disparar el reinicio
Cerrado de T3:
leído: clangd process alive

T4 preguntó:
dónde se llama a restart lsp for workspace cuando el proceso LSP muere
Cerrado de T4:
leído: clangd process alive
