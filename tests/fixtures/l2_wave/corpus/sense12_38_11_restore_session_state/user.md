...
2. [paso] B locator: dónde se restaura el estado del editor al abrir el proyecto
3. [fallo] P puente: hay camino guardar y restaurar el estado del editor (need A B)
...
T3 preguntó:
hay camino guardar y restaurar el estado del editor
Cerrado de T3:
ESA pregunta no se relanza. Lo leído no era el disparo. PROHIBIDO el mismo claim. do=cerrar ese mapeo. Si el ancla pedía más, un objeto distinto del pack; no el mismo claim en otro keep; no refutes el claim entero.
no encontré el mecanismo de guardar restaurar estado del editor; lo leído son funciones de gestión de cursores reset to single cursor, set primary y la estructura Editor Buffer, sin serialización ni persistencia
...
consulta = mecanismo (escritura a medias, reversión), no una categoría ('los archivos'). Si este tiro fue rechazado, el siguiente es explorar, no otro plan. El runtime no redacta la consulta.
