...
1. [en_curso] A locator: dónde se guarda el estado de visibilidad de los paneles laterales al cerrar la sesión
2. [pendiente] B locator: dónde se restaura la visibilidad de los paneles laterales al cargar la sesión (hacia layout)
3. [pendiente] P puente: hay camino guardar y restaurar la visibilidad de los paneles laterales (need A B)
...
T3 preguntó:
dónde se restaura la lista de archivos abiertos al cargar la sesión del proyecto
Cerrado de T3:
encontré el mecanismo: restore workspace session itera session open tabs y llama a workspace open file para cada archivo válido.

T4 preguntó:
dónde se guarda el estado de visibilidad de los paneles laterales al cerrar la sesión
Cerrado de T4:
ESA pregunta no se relanza. Lo leído no era el disparo. PROHIBIDO el mismo claim. do=cerrar ese mapeo.
no encontré el mecanismo de persistencia del estado de visibilidad de los paneles laterales; lo leído en save workspace session solo guarda rutas de tabs abiertos y argumentos de lanzamiento, sin tocar Main Layout State ni los latches de visibilidad visible etc.
