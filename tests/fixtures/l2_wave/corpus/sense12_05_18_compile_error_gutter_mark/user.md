...
1. [en_curso] A locator: dónde se parsean los errores de compilación del texto de la consola para extraer línea y columna
2. [pendiente] B locator: dónde se crean los objetos de diagnóstico a partir de los errores parseados
3. [pendiente] P puente: hay camino el parser de errores de la consola y la creación de diagnósticos del editor (need A B)
...
T1 preguntó:
dónde se extraen la línea y el número de columna de los errores de compilación del texto de la consola
Cerrado de T1:
Lo leído no contestó ESA pregunta. Un hop de Abierto/extra no es un hueco.
no encontré la extracción de línea y columna de errores de compilación; lo leído es un escáner de errores de enlazador linker que extrae solo símbolos indefinidos, no coordenadas de fuente
...
T3 preguntó:
dónde se crea el objeto de diagnóstico que contiene la línea y columna del error para que el editor lo pinte
Cerrado de T3:
leído: diagnostics on line, diagnostics for editor line, diagnostics hpp, editor panel hpp

T4 preguntó:
dónde se parsean los errores de compilación del texto de la consola para extraer línea y columna
Cerrado de T4:
