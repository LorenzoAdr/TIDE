...
1. [en_curso] A locator: dónde se guarda el estado de la sesión al cerrar el proyecto
2. [pendiente] B locator: dónde se restaura el estado de la sesión al abrir el proyecto
3. [pendiente] P puente: hay camino guardar y restaurar el estado de la sesión (need A B)
...
T3 preguntó:
hay camino guardar la posición del cursor y restaurarla al abrir
Cerrado de T3:
ESA pregunta no se relanza. Lo leído no era el disparo. PROHIBIDO el mismo claim. do=cerrar ese mapeo. Si el ancla pedía más, un objeto distinto del pack; no el mismo claim en otro keep; no refutes el claim entero.
no encontré el camino de guardar restaurar la posición del cursor al abrir; lo leído confirma que no hay flujo directo desde open file impl a set primary, y el sistema Editor Buffer no expone mecanismos de persistencia de cursor en el header ni en los verbos leídos

T4 preguntó:
dónde se guarda el estado de la sesión al cerrar el proyecto
Cerrado de T4:
leído: save workspace session, save, flush active tab, Workspace Session, workspace model hpp
