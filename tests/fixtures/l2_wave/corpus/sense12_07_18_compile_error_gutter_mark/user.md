...
1. [paso] A locator: dónde se extrae la línea de error del texto de la consola inferior
2. [fallo] B locator: dónde se dibuja la línea roja en el margen del editor (hacia visual highlight)
3. [pendiente] P puente: hay camino el error de la consola y el dibujo del margen (need A B)

Trabajos ya hechos:
### Trabajo 1
consulta: dónde se extrae la línea de error del texto de la consola inferior
keep: M7
leído: scan shell output for linker errors, parse pty filter tokens
Cerrado:
leído: scan shell output for linker errors, parse pty filter tokens
Abierto:
...
T1
    visto: scan shell output for linker errors, parse pty filter tokens
...
entre abiertas:
  T1=>T2  mismo objeto: scan shell output for linker errors
hacia el resto:
