...
1. [paso] A locator: dónde se dibuja una línea roja en el margen izquierdo del editor para marcar un error
2. [fallo] B locator: dónde se parsean los errores de compilación de la consola para obtener la línea del error (hacia parseo)
3. [pendiente] P puente: hay camino parsear el error de la consola y dibujar la línea roja en el editor (need A B)
...
T2 preguntó:
dónde se parsean los errores de compilación de la consola para obtener la línea del error
Cerrado de T2:
ESA pregunta no se relanza. Lo leído no era el disparo. PROHIBIDO el mismo claim. do=cerrar ese mapeo. Si el ancla pedía más, un objeto distinto del pack; no el mismo claim en otro keep; no refutes el claim entero.
no encontré el mecanismo de parsing de errores de compilación de la consola; lo leído son funciones de parsing de código fuente con Tree-sitter parse file, parse sync que construyen ASTs, no analizadores de output de compilador
