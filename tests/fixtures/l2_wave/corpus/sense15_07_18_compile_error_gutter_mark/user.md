...
bosquejo (no es el código; no copies nombres ni ids):
  olores:
    ...
    diagnóstico compilación ~ ui (c=0.57, twin lsp)
    línea error ~ (sin olor)
...
1. [en_curso] A locator: dónde se envían los diagnósticos de compilación desde la consola al editor
2. [pendiente] B locator: dónde se suscribe el editor a los cambios de diagnósticos para dibujar marcas
3. [pendiente] P puente: hay camino el envío de diagnósticos de la consola y la suscripción del editor para dibujar marcas (need A B)
...
T1 preguntó:
dónde se detectan los errores de compilación en la consola
Cerrado de T1:
encontré el objeto de la consulta: la detección de errores de compilación en la consola se realiza en build rows que ingesta los diagnósticos del LSP vía ISymbol Provider y los filtra por severidad k Error lógica en count errors siendo luego renderizados por Make Diagnostics Panel dentro de Make Console Panel

T2 preguntó:
dónde se dibujan las marcas visuales en el margen del editor
Cerrado de T2:
ESA pregunta no se relanza. Lo leído no era el disparo. PROHIBIDO el mismo claim. do=cerrar ese mapeo. Si el ancla pedía más, un objeto distinto del pack; no el mismo claim en otro keep; no refutes el claim entero.
leído: collect diagnostic decorations, apply decoration, render rich line, line gutter marker, fold gutter marker

T3 preguntó:
dónde se envían los diagnósticos de compilación desde la consola al editor
Cerrado de T3:
ESA pregunta no se relanza. Lo leído no era el disparo. PROHIBIDO el mismo claim. do=cerrar ese mapeo. Si el ancla pedía más, un objeto distinto del pack; no el mismo claim en otro keep; no refutes el claim entero.
leído: parse diagnostic, parse publish diagnostics, on lsp notification, set diagnostics notify callback, Application
