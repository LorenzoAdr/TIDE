...
1. [en_curso] A locator: dónde se parsean los errores de compilación de la salida de la consola para extraer la línea y el mensaje
2. [pendiente] B locator: dónde se crean las marcas de diagnóstico en el margen del editor a partir de los errores extraídos
3. [pendiente] P puente: hay camino el parseo de errores de la consola y la creación de marcas de diagnóstico en el margen (need A B)
...
T1 preguntó:
dónde se procesa la salida de texto de la consola inferior para extraer mensajes
Cerrado de T1:
leído: rebuild display, styled rows, rebuild cache, rebuild display locked, scan shell output for linker errors, drain output bytes

T2 preguntó:
dónde se dibujan las marcas de colores en el margen izquierdo del editor
Cerrado de T2:
leído: build git marks snapshot

T3 preguntó:
dónde se actualiza el estado de las marcas visuales en el margen del editor para reflejar errores
Cerrado de T3:
leído: diagnostics on line, diagnostics for editor line, rebuild diagnostics by line, rebuild diagnostics by line if changed

T4 preguntó:
dónde se parsean los errores de compilación de la salida de la consola para extraer la línea y el mensaje
Cerrado de T4:
leído: scan shell output for linker errors
