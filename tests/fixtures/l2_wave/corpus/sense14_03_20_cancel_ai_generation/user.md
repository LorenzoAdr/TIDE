...
peek: level2 session reopen for followup, level2 session apply tool, level2 session apply a judge
...
peek: file picker MakeFilePickerOverlay, file picker cancel ctrl chord, file picker arm ctrl chord
...
owns: console panel
nucleus: b row
peek: console panel copy terminal selection, console panel terminal selected text, console panel normalize terminal selection
...
owns: level1 agent
nucleus: ctx
peek: level1 agent filter distilled ignore, level1 agent run, level1 agent propose investigate needles
port: M6=>M8 ai controller cancel level1 -call-> workspace config load
M7  kind=chrome  ov=4  stems: editor panel editor panel
owns: editor panel
nucleus: scroll
peek: editor panel MakeEditorPanel, editor panel handle breadcrumb click, editor panel clear hover state
...
peek: ai controller begin insert at, ai controller cancel level1, ai controller cancel current
...
owns: app session
gap: no state
peek: raw pty screen skip escape, raw pty screen handle csi, raw pty screen feed
...
owns: gdb launcher
gap: no state
peek: git service unbind remote cancel, thread safe queue reset, closed 
...
1. [fallo] A locator: dónde se captura la pulsación de Escape o el clic fuera para interrumpir la generación de la IA
2. [fallo] B locator: dónde se limpia el archivo parcialmente escrito al cancelar la generación de la IA
3. [pendiente] P puente: hay camino capturar la interrupción y limpiar el archivo parcialmente escrito (need A B)
...
T1 preguntó:
dónde se captura la pulsación de Escape o el clic fuera para interrumpir la generación de la IA
Cerrado de T1:
ESA pregunta no se relanza. Lo leído no era el disparo. PROHIBIDO el mismo claim. do=cerrar ese mapeo. Si el ancla pedía más, un objeto distinto del pack; no el mismo claim en otro keep; no refutes el claim entero.
leído: cancel level1

T2 preguntó:
dónde se limpia el archivo parcialmente escrito al cancelar la generación de la IA
Cerrado de T2:
ESA pregunta no se relanza. Lo leído no era el disparo. PROHIBIDO el mismo claim. do=cerrar ese mapeo. Si el ancla pedía más, un objeto distinto del pack; no el mismo claim en otro keep; no refutes el claim entero.
no encontré la limpieza del archivo parcialmente escrito; lo leído clear pending insert solo resetea flags en memoria pending insert, insert anchor y no toca el sistema de archivos
