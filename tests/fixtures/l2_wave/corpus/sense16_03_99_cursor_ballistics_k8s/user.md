...
bosquejo (no es el código; no copies nombres ni ids):
  olores:
    ...
    índice vectorial ~ (sin olor)
    parpadeo usuario ~ (sin olor)
...
no encontré el cálculo de trayectoria balística ni sincronización con Kubernetes; lo leído es un editor de texto local editor panel con lógica de scroll y breadcrumbs, sin mención a clusters ni física de cursores
Abierto:
- balistic cursor calc
- k8s sync
