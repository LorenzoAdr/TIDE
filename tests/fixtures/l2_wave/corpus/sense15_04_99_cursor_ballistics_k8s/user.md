...
bosquejo (no es el código; no copies nombres ni ids):
  olores:
    trayectoria cursor ~ terminal (c=0.46)
...
port: M5=>M2 application handle focus shortcuts -call-> ui panel render cache mark dirty
...
M7  kind=caller  ov=0  stems: binary symbols panel binary symbols panel
owns: binary symbols panel
nucleus: filter dirty, status
peek: binary symbols panel MakeBinarySymbolsPanel, binary symbols panel poll runner
port: M7=>M1 visual highlight compute -call-> ai controller clear
...
M10  kind=hole  ov=21  stems: shell session diagnostics external file conflict search panel
owns: shell session / diagnostics panel MakeDiagnosticsPanel
gap: no state
...
bridges: T1[M9,M5] T2[M2,M1] T3[M9,M5] T4[M10,M2]
...
no encontré el cálculo de trayectoria balística ni sincronización con Kubernetes; lo leído es un editor de texto local con gestión de cursores y completado
Abierto:
