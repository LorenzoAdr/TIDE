...
M9  kind=hole  ov=9  stems: application tr workspace session workspace model
owns: application / workspace model open new tab from disk
gap: no state
peek: workspace model open new tab from disk, workspace model save buffer
peek-edge: workspace model stamp tab disk mtime -call-> workspace model file mtime seconds
...
hacia el resto:
M4=>M5 application handle focus shortcuts -call-> console panel cycle console tab
M5=>M4 application handle focus shortcuts -call-> console panel cycle console tab
...
kind=caller
owns: application
nuclei: C1(ai indexes requested )
anchors:
- G1: application sync symbol workspace indexer[q4]
closure: 1/1 groups, hub-free=1
roles:
- writers: application sync symbol workspace indexer, application request ai indexes, application set workspace
- readers: application sync symbol workspace indexer
- controls: application L862:guard
mechanism:
- trigger: application sync symbol workspace indexer -enter ctrl-> application L862:guard if (!force && !ai indexes requested )
- state: application sync symbol workspace indexer -write(ai indexes requested )-> ai indexes requested 
- effect: application sync symbol workspace indexer -call-> symbol workspace indexer start scan
ports:
- M4=>M5 application handle focus shortcuts -call-> console panel cycle console tab
causal edges:
- application sync symbol workspace indexer -write(ai indexes requested )-> ai indexes requested  [state]
- application sync symbol workspace indexer -enter ctrl-> application L862:guard [trigger] if (!force && !ai indexes requested )
- application sync symbol workspace indexer -call-> symbol workspace indexer start scan [effect]
- application handle focus shortcuts -call-> console panel cycle console tab [port]
mini-cards:
- application sync symbol workspace indexer | void Application sync symbol workspace indexer(bool force) | roles=mutator | writes=ai indexes requested  | hot=early return,multi return
· 854: symbol indexer .stop();
· 869: const auto snap = symbol indexer .snapshot();
· 877: const auto file snap = indexer .snapshot();
· 887: mapped files.reserve(snap->symbols.size());
- application set workspace | void Application set workspace(const std string &workspace root, | roles=mutator,io,ui | writes=workspace .status message,open hint,ai indexes requested  | reads=path,tr,active environment fingerprint | hot=file io,ui event,symptom edge
- application request ai indexes | void Application request ai indexes() | roles=mutator | writes=ai indexes requested 
## zone bridges
