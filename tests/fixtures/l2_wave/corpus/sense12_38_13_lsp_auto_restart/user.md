...
Cerrado:
encontré el mecanismo de reinicio del LSP: se dispara por cambio de entorno process build environment updates o configuración apply workspace settings, y se ejecuta en restart lsp for workspace
Abierto:
...
Preguntó: dónde se detecta que el servidor de lenguaje LSP se ha caído o dejado de responder
...
Esta pregunta ya tiene respuesta (no hay objeto). No la relances ni en otro keep. Si el ancla era esa pregunta, cierra el claim citando lo leído. Si el ancla pedía más, un objeto distinto del pack; no refutes el claim entero. Ya se tiró: «dónde se detecta que el servidor de lenguaje LSP se ha caído o dejado de responder». Rechazado: dónde se detecta que el servidor de lenguaje LSP se ha caído o dejado de responder
