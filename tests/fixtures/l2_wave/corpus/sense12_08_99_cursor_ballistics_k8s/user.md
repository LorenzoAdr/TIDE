...
M12  kind=hole  ov=9  stems: ui activity gate symbol workspace indexer busy strip application
owns: ui activity gate
gap: no state
peek: symbol workspace indexer progress, application run custom event drain
peek-edge: application run custom event drain -call-> symbol workspace indexer progress
