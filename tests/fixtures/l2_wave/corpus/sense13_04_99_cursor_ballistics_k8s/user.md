...
M11  kind=hole  ov=15  stems: ui perf monitor ui panel render cache ui activity gate theme
...
M12  kind=hole  ov=0  stems: language server spec language server spec
...
bridges: T1[M9,M5] T3[M9,M5] T4[M11,M10,M2] T5[M10,M2,M11]
holes: model store llama bundle dir for workspace model stamp tab disk mtime diagnostics panel MakeDiagnosticsPanel diagnostics count workspace diagnostics

Trabajos ya hechos:
(ninguno)

Legal ahora: bosquejar, explorar, ampliar, plan, cerrar
Aún no hay exploradores. El pack son las fichas. Si ov empatado o no hueles un mecanismo en owns/núcleo, amplia 1–3 M*. bosquejar nombra olores de zona si no ves un objeto; no es un job. No copies el ancla: aún no hay inspect. El prompt de investigación sale de lo visto. Si el ancla enumera, la consulta no enumera. PROHIBIDO copiar ids.
