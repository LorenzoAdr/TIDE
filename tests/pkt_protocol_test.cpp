...
#ifdef TGDB_SOURCE_DIR
      fs::path(TGDB_SOURCE_DIR) / relative,
#endif
...
int main() {
  ...
  tgdb::packet_monitor::ProtocolDefinition protocol;
  std::string error;
  if (!tgdb::packet_monitor::load_protocol_from_json(protocol_path.string(), &protocol, &error)) {
  ...
  const auto decoded_hb =
      tgdb::packet_monitor::decode_packet(heartbeat, protocol, "0x01");
  ...
  const auto decoded_sensor =
      tgdb::packet_monitor::decode_packet(sensor, protocol, "0x02");
