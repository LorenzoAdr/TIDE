...
### turn 8 — `search` `command_exists`

```
q=command_exists
/home/lariasr/workspace/TIDE/tests/search_replace_test.cpp:60:        "#pragma once\n\n#include <string>\n\nnamespace tuide {\n\nbool command_exists();\n\n}\n";
/home/lariasr/workspace/TIDE/tests/search_replace_test.cpp:62:        "#pragma once\n#include <string>\nnamespace tuide {\nbool command_exists();\n}\n";
/home/lariasr/workspace/TIDE/tests/search_replace_test.cpp:72:                "// L2_PS_A\nbool command_exists();\n\n}\n";
/home/lariasr/workspace/TIDE/tools/l2_battery/facets/clarify_calib/cases.json:59:    "prompt": "En src/util/shell_utils.cpp command_exists añade // L2_CALIB_PROBE antes del return con std::system.",
/home/lariasr/workspace/TIDE/src/build/compile_commands_generator.cpp:221:  if (command_exists("bear")) {
/home/lariasr/workspace/TIDE/src/build/compile_commands_generator.cpp:224:  if (command_exists("compiledb")) {
/home/lariasr/workspace/TIDE/src/build/compile_commands_generator.cpp:232:  if (!command_exists("bear")) {
/home/lariasr/workspace/TIDE/src/build/compile_commands_generator.cpp:250:  if (!command_exists("compiledb")) {
/home/lariasr/workspace/TIDE/src/build/compile_commands_generator.cpp:281:  if (environment.docker_container.empty() || !command_exists("docker") ||
/home/lariasr/workspace/TIDE/src/build/compile_commands_generator.cpp:282:      !command_exists("bear")) {
/home/lariasr/workspace/TIDE/src/build/docker_environment_detector.cpp:122:  if (workspace_root.empty() || !command_exists("docker")) {
/home/lariasr/workspace/TIDE/tools/l2_battery/prompts_hard.json:4:    "prompt": "Añade bool always_true() en src/util/shell_utils.hpp (declaración junto a command_exists) y su definición en src/util/shell_utils.cpp que retorne true. Debes modificar ambos archivos. No toques otras funciones."
/home/lariasr/workspace/TIDE/tools/l2_battery/prompts_hard.json:19:    "id": "05_command_exists_marker",
/home/lariasr/workspace/TIDE/tools/l2_battery/prompts_hard.json:20:    "prompt": "En src/util/shell_utils.cpp, dentro de command_exists, añade una sola línea de comentario // L2_HARD_PROBE inmediatamente antes del return que llama a std::system. No cambies la lógica."
… (observation truncated at 2400 chars)

note: [TRUNCATED] — usa `refetch` (`path:A-B` / `path:Symbol#mid|#tail`); no inventes código.
```

_nudge:_ phase=edit: llevas 3 tools. Emite **`action=edit`** ahora con `search` = span completo del pack (p.ej. `struct Foo {` … `};`), path del fragmento. No más `get_code_of` en bucle.

### turn 9 — edit_feedback

error: hunk mal formado: `search` es solo el opener (`… {`) pero `replace` trae el bloque entero — el `search` debe cubrir todo el span a sustituir (p.ej. struct completo hasta `};`), no solo la línea de apertura
...
Reemite `action=edit` con `search` exacto y único (bloque de código, no un ident suelto; preferir multilínea). Si cambias un `struct`/`class`, el `search` debe cubrir **todo** el span hasta `};`, no solo `struct X {`. Usa `get_code_of`. **No repitas el mismo hunk fallido.** Si introduces un símbolo nuevo, incluye también el hunk de declaración en el `.hpp`/`.h` hermano.

**Siguiente acción: `action=edit`** (evita bucle de tools).

## span sugerido desde pack (úsalo como base del search)

```
src/util/shell_utils.hpp:1-13 (shell_quote)
symbol_span: 7-7
#pragma once
