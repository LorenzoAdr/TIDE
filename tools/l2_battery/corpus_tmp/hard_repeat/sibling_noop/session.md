...
… (observation truncated at 40 lines)
note: [TRUNCATED] — usa `refetch` del resultado (`path:A-B` / `path:Symbol#mid|#tail`); no inventes el cuerpo.
```

### turn 11 — `get_code_of` `src/util/shell_utils.hpp`

```
src/util/shell_utils.hpp:7-7 (shell_quote)
symbol_span: 7-7
std::string shell_quote(const std::string& value);
```

### turn 12 — `get_code_of` `src/util/shell_utils.cpp`

```
src/util/shell_utils.cpp:9-20 (shell_quote)
symbol_span: 9-20
std::string shell_quote(const std::string& value) {
  std::string quoted = "'";
...
### turn 13 — edit_feedback

error: hunk falló (src/util/shell_utils.hpp): search no encontrado (0 matches) — revisa path (candidates pack/watchlist: `src/util/shell_utils.hpp`, `src/util/shell_utils.cpp`); usa newlines reales (no `\s*`/`\n` literales)
...
## on disk (copia literal al `search`; respeta líneas en blanco)
