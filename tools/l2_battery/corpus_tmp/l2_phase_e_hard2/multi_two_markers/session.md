...
query: En src/util/shell_utils.hpp: (1) // L2_PS_A encima de shell_quote; (2) // L2_PS_B encima de command_exists. Ambos obligatorios.
...
<!-- map compacted: full detail for 3 hot stem(s); other entries are name-only -->
## Observations

<!-- older observations trimmed (4) -->

ght_sidebar_panel.cpp`
...
### fresh `src/ui/main_layout.hpp:shell_utils`

```
src/ui/main_layout.hpp:1-448 (shell_utils) [TRUNCATED]
symbol_span: 1-448
sent: 1-448
missing_lines: 67-394
refetch: get_code_of `src/ui/main_layout.hpp:67-186` (o `#head`/`#mid`/`#tail` / `path:A-B`)
note: cuerpo incompleto — usa refetch; no inventes el código omitido.
#pragma once
...
Explorando targets para encontrar L2_PS_A y L2_PS_B

target_count: 5  fragments_ok: 5/6  outlines_ok: 3/3  truncated: 2  pack_chars: 7659/9000  auto_refetch: 1
...
_nudge:_ Llevas 3 `plan` seguidos con pack cubierto. Emite `done next=edit` o `edit` (no más `plan` sin tools/`get_code_of` nuevos).

### turn 15 — done_coverage_gate
