#!/usr/bin/env bash
# BD: N reps post ablación C (minimal + veredicto hijo B + job_cap D; polos off).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$ROOT"
N="${1:-5}"
BASE="${2:-$ROOT/.tuide/ai/l2_wave/v2_night/BD}"
...
if [[ "${BD_DETACHED:-}" != "1" ]]; then
  if [[ -f "$PIDFILE" ]]; then
    old="$(tr -d ' \n' < "$PIDFILE" || true)"
    if alive "$old"; then
      echo "BD queue already running pid $old"
      exit 0
    fi
  fi
  export BD_DETACHED=1
  setsid "$0" "$@" >>"$LOG" 2>&1 < /dev/null &
  echo $! > "$PIDFILE"
  echo "BD queue detached pid $! n=$N base=$BASE"
  exit 0
fi

echo $$ > "$PIDFILE"
echo "==== BD queue start $(date -Iseconds) n=$N diet=minimal+B+D C=off ====" | tee -a "$BASE/STARTED.txt"
if [[ ! -x "$CLI" ]]; then
...
for i in $(seq -w 1 "$N"); do
  ...
  echo "==== BD rep_$i $(date -Iseconds) ====" | tee -a "$OUT/STARTED.txt"
...
if [[ -f "$BASE/A4_summary.json" ]]; then
  mv -f "$BASE/A4_summary.json" "$BASE/BD_summary.json"
fi
python3 "$ROOT/tools/l2_wave/compare_a4_s0.py" \
  "$ROOT/.tuide/ai/l2_wave/v2_night/S0" "$BASE" \
  -o "$ROOT/.tuide/ai/l2_wave/v2_night/BD_vs_S0.json" || true

echo "==== BD queue done $(date -Iseconds) ====" | tee -a "$BASE/STARTED.txt"
touch "$BASE/DONE"
rm -f "$PIDFILE"
python3 "$ROOT/tools/l2_wave/decide_vs_s0.py" \
  "$ROOT/.tuide/ai/l2_wave/v2_night/S0" "$BASE" --label BD \
  -o "$ROOT/.tuide/ai/l2_wave/v2_night/DECISION_BD.json" || true
printf '%s %s\n' "AGENT_LOOP_WAKE_v215h" \
  '{"prompt":"Lee y ejecuta tools/l2_wave/v2_15h_tick_prompt.txt (noche 15h: BD done).","why":"bd_done"}'
